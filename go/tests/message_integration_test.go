...
func TestCompleteMessageWorkflow(t *testing.T) {
	...
	originalMsg.Init(
		...
		"",
		time.Now().Unix(), time.Now().Unix()+5, time.Now().Unix()+10, time.Now().Unix()+15, 30, 0, false,
	)
...
func verifyMessageEquality(t *testing.T, original, reconstructed *ifs.Message) {
	...
	if reconstructed.Tr_Created() != original.Tr_Created() {
		t.Errorf("Transaction created mismatch: expected %d, got %d", original.Tr_Created(), reconstructed.Tr_Created())
	}
	if reconstructed.Tr_Queued() != original.Tr_Queued() {
		t.Errorf("Transaction queued mismatch: expected %d, got %d", original.Tr_Queued(), reconstructed.Tr_Queued())
	}
	if reconstructed.Tr_Running() != original.Tr_Running() {
		t.Errorf("Transaction running mismatch: expected %d, got %d", original.Tr_Running(), reconstructed.Tr_Running())
	}
	if reconstructed.Tr_End() != original.Tr_End() {
		t.Errorf("Transaction end mismatch: expected %d, got %d", original.Tr_End(), reconstructed.Tr_End())
	}
}
...
func TestMultipleRoundTrips(t *testing.T) {
	...
	msg := &ifs.Message{}
	msg.Init("dest", "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte("initial data"), true, false, 123, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
	...
	for i := 0; i < 10; i++ {
		go func(index int) {
			...
			msg.Init(
				...
				int64(index*10000), int64(index*10000+100), int64(index*10000+200), int64(index*10000+300), 30, 0, false,
			)
...
func TestMessageSizeCalculation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.trState != ifs.NotATransaction {
			...
			msg.Init("dest", "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte(data), true, false, 123, tc.trState, trId, trErr, trCreated, trQueued, trRunning, trEnd, trTimeout, 0, false)
...
func BenchmarkMessageMarshal(b *testing.B) {
	resources := newMockResources()
	msg := &ifs.Message{}
	msg.Init("dest", "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte("benchmark data"), true, false, 123, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
