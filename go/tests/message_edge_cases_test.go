...
func TestMessageBoundaryConditions(t *testing.T) {
	...
	msg.Init(
		...
		"",
		9223372036854775807, 9223372036854775806, 9223372036854775805, 9223372036854775804, 30, 0, false, // max int64 values
	)
...
func TestMessageDataSizes(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			msg := &ifs.Message{}
			msg.Init("dest", "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte(tc.data), true, false, 123, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
...
func TestTransactionErrorMessages(t *testing.T) {
	...
	for _, errMsg := range testCases {
		...
		msg.Init(
			...
			true, false, 123, ifs.Failed, "tr-id", errMsg, 1234567890, 1234567900, 1234567910, 1234567920, 30, 0, false,
		)
...
func TestServiceNameLengthHandling(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			msg := &ifs.Message{}
			msg.Init("dest", tc.serviceName, 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte("data"), true, false, 123, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
...
func TestPredefinedDestinations(t *testing.T) {
	...
	for _, destination := range testCases {
		msg := &ifs.Message{}
		msg.Init(destination, "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte("data"), true, false, 123, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
...
func TestSequenceNumberOverflow(t *testing.T) {
	...
	for _, seq := range testSequences {
		msg := &ifs.Message{}
		msg.Init("dest", "service", 1, ifs.P1, ifs.M_All, ifs.POST, "source", "vnet", []byte("data"), true, false, seq, ifs.NotATransaction, "", "", 0, 0, 0, 0, 0, 0, false)
